...
- **Status:** Akzeptiert, in Umsetzung (Kern/Entscheidungen 1–4 umgesetzt: `allowedSources` pro Schlüssel im Schlüsselbund (ADR-025), Server setzt/validiert die `source` beim Write — eine erlaubte Source darf der Client weglassen, abweichende/nicht erlaubte → harte Ablehnung (403; fehlende Pflicht-source bei mehreren erlaubten → 400); exaktes Matching; Widerruf über den bestehenden Keyring. Inbox (5) und Promotion (6) bewusst als Folge-Schritt offen.)
