...
- **Status:** Abgelöst durch ADR-034 (akzeptiert 2026-06-24)
...
- **Abgelöst:** Der Partitionierungs-Cluster (ADR-034…038) erlaubt horizontale Skalierung über mehrere Knoten. Die Single-Instance bleibt jedoch der **voreingestellte** Betriebsmodus (`CLIO_PARTITIONS=1`, Coordinator `static`) und ist dort verhaltensgleich zu dieser ADR — Clustering ist opt-in (siehe Designprinzip §4.1). Diese ADR beschreibt damit weiterhin den Default-Modus, ist als *globale* Festlegung „kein Clustering" aber abgelöst.
...
- **Abgelöst:** ADR-034 ersetzt die *eine globale* serialisierte Stelle durch **Serialisierung pro Partition** (Single-Writer-per-Partition, eigene Sequenz/Kette je Partition). Die globale Total-Order entfällt (INV-P1); die per-Partition-Serialisierung und monotone ID-Vergabe **innerhalb** einer Partition leben unverändert weiter. Bei `CLIO_PARTITIONS=1` ist das Verhalten identisch zu dieser ADR.
...
- **Einzeldokument:** [`docs/adr/0026-authentifizierte-event-herkunft-ueber-tokens.md`](./docs/adr/0026-authentifizierte-event-herkunft-ueber-tokens.md) — als Einzeldatei ausgelagerte, fortan maßgebliche Fassung (Body wörtlich identisch zum folgenden Abschnitt).
...
- **Status:** Akzeptiert · **umgesetzt** (Registry, Middleware-Verdrahtung, Config, opt-in Lifecycle-Events, Metriken, `GET /api/v1/activity` und `/ui`-Tab „Aktivität"). Umsetzungsplan: [`docs/plans/activity-presence-plan.md`](./docs/plans/activity-presence-plan.md)
...
- **Entscheidung:** Drei nach Last-Profil getrennte Schichten:
  ...
  2. **Auth-Lifecycle als CloudEvents (Dogfooding), opt-in.** Die *seltenen, wertvollen* Ereignisse — **Session-Start/-Ende** (das „Login"-Äquivalent: der Übergang offline→online bzw. dessen Ablauf), **Key angelegt/widerrufen** — werden als echte Events in einen reservierten, **server-only** Subject-Namespace `/_clio/auth/…` geschrieben (direkt über den Store, nicht über die HTTP-Schicht → kein Auth-Pfad, keine Rekursion, kein Henne-Ei-Problem). Damit sind Login-Zeiten und Schlüssel-Lebenszyklus **mit dem vorhandenen Werkzeug** abfragbar/beobachtbar — ohne neuen Lese-Endpunkt. Geschaltet über `CLIO_AUTH_EVENTS` (Default **aus**, konsistent mit `CLIO_EVENT_AUTHORSHIP`: ein Eingriff in den Event-Strom des Betreibers ist eine bewusste Wahl). `data` trägt nur nicht-geheime Felder (`kid`, `name`, `scopes`, `reason`), **nie** ein Secret/Hash und **keine** Remote-IP. Der `/_clio/`-Prefix ist für Client-Writes gesperrt (sonst ließen sich Login-Events fälschen).
  3. **Admin-Sichten.** Neuer Endpunkt `GET /api/v1/activity` (Scope `admin`) liefert den Presence-/Aktivitäts-Snapshot; ein neuer **UI-Tab „Aktivität"** (ADR-020-konform: Vanilla JS, kein Build-Step) zeigt online-Status live, eine Aktivitätstabelle je `kid` und einen Live-Feed der Lifecycle-Events (über `observe` auf `/_clio/auth/`). Die **Historie** kommt aus den `/_clio/auth/`-Events, nicht aus einem neuen Speicher.
- **Konsequenzen (positiv):** Betreiber sehen, wer online ist und wer was tut, ohne neue Infrastruktur. Die Auth-Historie ist unveränderlich (Hash-Kette/Signatur, ADR-012/016) und mit denselben Werkzeugen abfragbar wie Fachdaten — konsequentes Dogfooding. Kein DB-Wachstum/Write-Last im Default (alles opt-in bzw. in-memory). Keine neue externe Abhängigkeit (ADR-001); Presence passt zur Single-Instance-Annahme (ADR-002).
- **Konsequenzen (negativ / Grenzen):** Presence geht beim Neustart verloren (bewusst — die dauerhafte Spur sind die Events). „Session" ist nur die Presence-Lücke, kein echtes Login/Logout. Aktiviert man `CLIO_AUTH_EVENTS`, wachsen die `/_clio/auth/`-Events mit (nie gelöscht, ADR-015) und sind bei rekursivem `read`/`observe` auf `/` sichtbar — für Transparenz erwünscht, ein optionaler Prefix-Ausschluss bleibt additiv nachrüstbar. **Bewusste Nicht-Ziele:** kein Event pro Read/Write (Event-Amplifikation; Reads sind der Hot-Path), keine Remote-IP/Geo/User-Agent-Erfassung per Default (Datensparsamkeit), kein RBAC über die bestehenden Scopes hinaus, keine echte Sitzungsverwaltung. `access-denied`-Events sind separat (`CLIO_AUTH_DENIED_EVENTS`) und rate-limitiert, damit ein Angreifer den Event-Strom nicht fluten kann.

### ADR-031: Backup/Restore/Verify über konsistente bbolt-Snapshots; PITR optional
- **Status:** Akzeptiert · **Stufe 1 umgesetzt** (`cliostore backup`/`restore`/`verify`, `backup --verify`, HTTP `GET /api/v1/backup`); Stufe 2 (Continuous Archiving / PITR) bewusst zurückgestellt. Detailkonzept: [`docs/plans/backup-restore-dr-concept.md`](docs/plans/backup-restore-dr-concept.md), Betriebsanleitung: [`docs/backup-restore.md`](docs/backup-restore.md).
- **Kontext:** Ein Event Store braucht eine glaubwürdige Antwort auf Verlustszenarien (Platte weg, Datei korrupt, Fehlbedienung). Es gab `verify` (ADR-012) und `compact` (ADR-015), aber keinen definierten Backup-/Restore-Weg. clio ist Single-Node auf einer bbolt-Datei (ADR-002/006); die Frage war, ob DR über *Replikation* (Kafka-artig) oder *Snapshots* (Postgres-artig) gelöst wird. *(Die ursprünglich im Konzeptdokument vorgeschlagene Nummer ADR-026 war zum Umsetzungszeitpunkt bereits durch die Event-Herkunft belegt — daher ADR-031.)*
...
### ADR-032: Persistentes Audit-Log administrativer Aktionen (separater bbolt-Bucket)
...
### ADR-033: Subject-/Prefix-basierte Scopes (`read:/orders/*`)
- **Status:** **Akzeptiert · umgesetzt** (`auth.ParseGrant`/`ScopesAllow`/`Key.HasAction`, Subject-Prüfung in den Datenroute-Handlern). Praxisbeispiele: [`docs/security.md`](docs/security.md). *(Abweichungen vom ursprünglichen Designvorschlag sind unten markiert.)*
- **Kontext:** Heute gilt ein Scope (`read`/`write`/`admin`/`audit`) **global** über alle Subjects. Für Mehrparteien-Szenarien (ein Producer darf nur `/orders/*` schreiben, ein Consumer nur `/orders/*` lesen) fehlt eine **feinere, subject-gebundene** Berechtigung. Ein volles RBAC/Policy-System widerspräche dem Single-Binary-/Stdlib-Prinzip (ADR-001) — gesucht ist die **kleinste** tragfähige Erweiterung. clio hat mit `MatchSubject` (rekursive Subtree-Prüfung, inkl. Ausschluss von Prefix-Geschwistern wie `/booksstore` zu `/books`) bereits den Kern-Baustein.
- **Entscheidung (Vorschlag):**
  1. **Scope-Grammatik, rückwärtskompatibel.** Ein Scope ist entweder global (`read`) — unverändertes heutiges Verhalten — oder **subject-gebunden** als `aktion:präfix`, z. B. `read:/orders/*`, `write:/orders/*`. Das `/*`-Suffix bezeichnet den **rekursiven Subtree** (`read:/*` = ganzer Baum); ohne `/*` gilt exakt das eine Subject. `admin`/`audit` bleiben **global** (Schlüssel-/Auditverwaltung ist nicht subjektgebunden). *(Abweichung vom Vorschlag: der separate Lese-Scope `observe` wurde **nicht** eingeführt — `read` deckt weiterhin History UND Live ab. Das hält die Änderung kleiner und vollständig rückwärtskompatibel; eine spätere observe-Trennung bleibt additiv möglich.)*
  2. **Datenmodell.** `auth.Key.Scopes` bleibt `[]Scope`, aber jeder Eintrag wird mit `ParseGrant` zu einem `Grant{Action, Subject, Recursive}` geparst. Neu: `Key.HasAction(action)` (Aktions-Gate), `ScopesAllow(scopes, action, subject, recursive)` / `Key.Allows(...)` (Subjekt-Prüfung) und `ValidScopeString` (Anlage-Validierung). Das Subtree-Matching ist eine kleine pure Funktion in `auth` (kein Storage-Import; Geschwister wie `/booksstore` zu `/books` werden über den `"/"`-Suffix-Vergleich korrekt ausgeschlossen). `HasScope` bleibt für globale Aktionen (`admin`/`audit`).
  3. **Durchsetzungspunkte.** Das Aktions-Gate bleibt in der Middleware (`requireScope` → `HasAction`: hat der Key **irgendeinen** Grant der Aktion?), die **Subjekt-Prüfung** wandert in die Handler (`s.authorizeSubject`), weil das Subjekt erst dort bekannt ist (Body/Query/Pfad): `read-events`/`observe-events`/`events`-Pfad (angefragtes Subject ⊆ Grant), `run-query` (Scope-Subtree ⊆ Grant), `write-events` (**jedes** Event-Subject ⊆ eines `write`-Grants — sonst 403, atomar nichts geschrieben).
  4. **Aggregat-/Metadaten-Routen** (`info`, `event-stats`, `read-event-types`, `read-event-schema`, `verify`, `read-subjects` ohne Prefix) verraten subjektübergreifende Information und verlangen daher einen **globalen** `read`-Grant (`s.authorizeGlobal`); `register-event-schema` verlangt globalen `write` (Schemas gelten typweit). Ein rein prefix-beschränkter Key bekommt dort 403. `read-subjects?prefix=…`/`children=…` sind erlaubt, wenn der Pfad ⊆ eines Grants ist. *(Ausnahme: `public-key` bleibt für jeden read-Grant offen — es verrät nur den nicht-geheimen Signatur-Public-Key. Dev-Routen sind weiterhin `admin`-global.)*
  5. **CLI/HTTP/Doku.** `cliostore keys create --scopes 'read:/orders/*,write:/orders/*'`; die Scope-Validierung (HTTP und CLI) akzeptiert die neue Grammatik über `ValidScopeString`; OpenAPI/`security.md` dokumentieren Beispiele.
- **Konsequenzen:** Least-Privilege bis auf Subtree-Ebene mit minimalem Code, ohne neue Abhängigkeit. Kosten: Die Autorisierung ist nicht mehr rein in der Middleware — jede Datenroute braucht eine **explizite** Subjekt-Prüfung (Risiko des Vergessens → durch den zentralen Helfer `authorizeSubject`/`authorizeGlobal` + Tests je Route abgefangen). Bewusste Nicht-Ziele: keine **Deny**-Regeln (nur additive Grants), keine Wildcards in der Mitte (`/orders/*/items`), keine attributbasierte Policy, keine Tenancy-/Datenraum-Isolation über die Subtree-Sichtbarkeit hinaus (Subjects bleiben in **einer** DB; das ist **keine** Mandantentrennung — siehe `docs/threat-model.md`). Migration ist nahtlos: bestehende globale Scopes verhalten sich unverändert (HasAction/Allows liefern für globale Grants dieselben Entscheidungen wie zuvor).
- **Bezug:** erweitert ADR-025 (Keys/Scopes), ergänzt die Aktivitäts-Instrumentierung der Middleware (ADR-030).
...
- Partitionierungsmodell für horizontale Skalierung (ADR-034, *akzeptiert 2026-06-24, Umsetzung offen*): bei 10⁹+ Events brechen Single-Writer (ADR-003), die globale Hash-Chain (ADR-012) und die bbolt-Single-File-Ablage (ADR-006). Vorschlag: Partitionierung entlang der `source`-Achse (ADR-004) per konsistentem Hashing, je Partition eigener Writer + eigene Hash-Chain + eigene Sequenz (Single-Writer-per-Partition). Folgenschwer: **globale Total-Order entfällt** (weicht bewusst von ADR-002/ADR-003 ab; neue Invariante **INV-P1**). ADR-Datei: [`docs/adr/0034-partitionierungsmodell-fuer-horizontale-skalierung.md`](./docs/adr/0034-partitionierungsmodell-fuer-horizontale-skalierung.md). Offen bleiben und je eigener Folge-ADR vorbehalten: (a) Anzahl/Granularität & Splitting/Merging der Partitionen; (b) `source → partition`-Mapping + Rebalancing (Distribution/Consensus); (c) übergeordnetes Anchoring der n Hash-Chains (Tamper-Evidence unter Partitionierung); (d) Storage-Engine jenseits bbolt; (e) Cross-Partition-Read-Modell (Read-Path/CQRS); (f) Hot-Partition-Mitigation via Sub-Partitionierung.
- Tamper-Evidence unter Partitionierung (ADR-035, *akzeptiert 2026-06-24, Umsetzung offen*): Folge-ADR zu ADR-034. Heute **eine** Hash-Chain (ADR-012); partitioniert zerfällt sie in **n** Ketten — rein per-Partition wären Drop/Rollback einer ganzen Partition unsichtbar. Vorschlag: je Partition eigene Kette **plus** ein periodischer **globaler Anker** (Merkle-Commitment über alle `(partitionID, head, seq)`, append-only Anker-Kette); globaler Verify = jede Kette intakt **und** Heads reproduzieren den letzten Anker. Anker-Koordinator liest nur (kein Schreib-Consensus, Single-Writer-per-Partition bleibt). Bestands-Migration über **versiegelte Epoche-0** statt Re-Chaining (wahrt ADR-012/015). Neue Invariante **INV-P2**. ADR-Datei: [`docs/adr/0035-tamper-evidence-unter-partitionierung.md`](./docs/adr/0035-tamper-evidence-unter-partitionierung.md). Offen: Anker-Kadenz (Default), verteilte Anker-Erzeugung (→ Distribution/Consensus), optionale externe Notarisierung (ADR-016).
- Read-Path & CQRS unter Partitionierung (ADR-036, *akzeptiert 2026-06-24, Umsetzung offen*): Folge-ADR zu ADR-034. Der Konsumenten-Audit zeigte, dass der externe Lese-/Cursor-Vertrag bricht (skalare globale ID als Cursor). Vorschlag: Cross-Partition-Reads/`run-query` als **Scatter-Gather** über die betroffenen Partitionen (single-source bleibt single-partition und damit günstig), **streaming k-Wege-Merge** (kein OOM, Resilienz ADR-028 gilt pro Partition), Ergebnis-Ordnung explizit als `per-partition`/`approximated` klassifiziert (nie global garantiert, INV-P1). Cursor wird ein **opaker per-Partition-Vektor** (`CLIO_PARTITIONS=1` = alter skalarer Cursor, abwärtskompatibel); globale Total-Order ist ein **nachgelagertes CQRS-Read-Modell**, kein Store-Feature. Neue Invariante **INV-P3**. ADR-Datei: [`docs/adr/0036-read-path-cqrs-unter-partitionierung.md`](./docs/adr/0036-read-path-cqrs-unter-partitionierung.md). Offen: internes Order-Read-Modell ja/nein (vgl. ADR-029), Kosten-/Index-Modell beim Scatter-Gather (ADR-023), Fan-out-/Tail-Latency-Begrenzung.
- Storage-Engine unter Partitionierung (ADR-037, *akzeptiert 2026-06-24, Umsetzung offen*): Folge-ADR zu ADR-034. Der `storage-scaling-plan` vermaß die Single-File-bbolt-Grenzen (Mmap-Remap, B+Tree-Rebalancing ab ~85 % Füllgrad). Vorschlag: **bbolt bleibt, aber eine Datei pro Partition** (kein Engine-Wechsel) — kleinere Datei ⇒ flacherer Tree, kleinere Mmap, kein gemeinsamer Schreib-Lock; die bewährte Toolchain (Verify ADR-012/035, Backup ADR-031, Compact ADR-015, Group Commit ADR-009) gilt pro Datei, ebenso die Scaling-Hebel des Plans. Pure-Go/Single-Binary (ADR-001) bleibt. Neu: **Handle-Pool (lazy open/close, LRU)** gegen Adressraum-/FD-Druck bei vielen Partitionen. Engine ist hinter der Partitions-Abstraktion **lokal austauschbar** (LSM für einzelne heiße Partitionen später möglich, jetzt verworfen). Keine neue Invariante (Mechanismus hinter INV-P1/P2/P3). ADR-Datei: [`docs/adr/0037-storage-engine-unter-partitionierung.md`](./docs/adr/0037-storage-engine-unter-partitionierung.md). Offen: Pool-Dimensionierung, Store-weiter konsistenter Snapshot über n Dateien, Engine-Wechsel-Kriterien.
- Distribution & Consensus unter Partitionierung (ADR-038, *akzeptiert 2026-06-24, Umsetzung offen*): letzte Folge-ADR zu ADR-034; löst `source → partition`-Mapping, Platzierung und Rebalancing über Knoten. Kernproblem: genau **ein** Writer pro Partition global (INV-P1) verlangt im verteilten Fall Koordination. Vorschlag: konsistentes Hashing (Mapping), Eigentümerschaft als **zeitlich begrenztes Write-Lease** mit fail-stop/fencing (neue Invariante **INV-P4**), Koordination hinter `Coordinator`-Schnittstelle mit Default **`static`** (Single-Node = heutiges ADR-002-Verhalten, **keine** neue Abhängigkeit) und opt-in **`raft`** (eingebettet, pure-Go — **kein** externer Dienst, wahrt ADR-001). Konsens nur über Membership/Leases, **nicht** den Event-Schreibpfad. Rebalancing = Bewegung **ganzer Partitionsdateien** (ADR-037). Bewusst **nicht** enthalten: Replikation/HA pro Partition (eigenes ADR); konkrete Raft-Bibliothek (wartet auf realen Multi-Node-Treiber). ADR-Datei: [`docs/adr/0038-distribution-consensus-partition-ownership.md`](./docs/adr/0038-distribution-consensus-partition-ownership.md).
- Aktivität & Presence (ADR-030, *vorgeschlagen*): „wer ist online / wer tut was" über eine in-memory Presence-Registry plus opt-in Auth-Lifecycle-Events (`/_clio/auth/…`, Dogfooding) und einen Admin-Endpunkt/`/ui`-Tab. Umsetzungsplan: [`docs/plans/activity-presence-plan.md`](./docs/plans/activity-presence-plan.md). Offen bleiben: (a) optionaler Ausschluss des `/_clio/`-Prefix aus rekursiven Wurzel-Reads, falls die Auth-Events als Rauschen empfunden werden; (b) optionale, datensparsame Erfassung von Remote-IP/User-Agent; (c) persistente Presence über Neustarts (bewusst zurückgestellt — die Lifecycle-Events sind die dauerhafte Spur).
