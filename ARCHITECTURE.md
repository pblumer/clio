...
### ADR-033: Subject-/Prefix-basierte Scopes (`read:/orders/*`) — Designvorschlag
- **Status:** **Vorgeschlagen** (Design, noch nicht implementiert). Dieser ADR legt das Zielbild fest, damit eine spätere Umsetzung klein und reviewbar bleibt; bis dahin sind Scopes global (ADR-025).
...
- **Entscheidung (Vorschlag):**
  1. **Scope-Grammatik, rückwärtskompatibel.** Ein Scope ist entweder global (`read`) — unverändertes heutiges Verhalten — oder **subject-gebunden** als `aktion:präfix`, z. B. `read:/orders/*`, `write:/orders/*`, `observe:/orders/*`. Das `/*`-Suffix bezeichnet den **rekursiven Subtree** (intern via `MatchSubject(subject, "/orders", true)`); ohne `/*` gilt exakt das eine Subject. `admin`/`audit` bleiben **global** (Schlüssel-/Auditverwaltung ist nicht subjektgebunden). *Neuer Lesescope `observe`* wird eingeführt, um Live-Streams getrennt vom Punkt-in-der-Zeit-Lesen vergeben zu können (heute deckt `read` beides ab; `observe` ohne Suffix = `read`-kompatibel).
  2. **Datenmodell.** `auth.Key.Scopes` bleibt `[]string`, aber jeder Eintrag wird zu einem `Grant{Action, Subject, Recursive, Global}` geparst. `Key.Allows(action, subject)` ersetzt `HasScope` an den Datenrouten; `HasScope` bleibt für globale Aktionen (`admin`/`audit`).
  3. **Durchsetzungspunkte.** Die Aktions-Gate bleibt in der Middleware (hat der Key **irgendeinen** Grant der Aktion?), die **Subjekt-Prüfung** wandert in die Handler, weil das Subjekt erst dort bekannt ist (Body/Query/Pfad): `read-events`/`observe`/`events`-Pfad (angefragtes Subject ⊆ Grant), `run-query` (Scope-Subtree ⊆ Grant), `write-events`/Bulk-Import (**jedes** Event-Subject ⊆ eines `write`-Grants — sonst 403, atomar nichts geschrieben). Ein kleiner Helfer `s.authorizeSubject(key, action, subject, recursive)` zentralisiert die Prüfung.
  4. **Aggregat-/Metadaten-Routen** (`read-subjects` ohne Prefix, `read-event-types`, `event-stats`, `verify`) verraten subjektübergreifende Information und verlangen daher einen **globalen** `read`-Scope (ein rein prefix-beschränkter Key bekommt dort 403). `read-subjects?prefix=…` ist erlaubt, wenn der Prefix ⊆ eines Grants ist.
  5. **CLI/HTTP/Doku.** `cliostore keys create --scopes 'read:/orders/*,observe:/orders/*'`; Scope-Validierung akzeptiert die neue Grammatik; OpenAPI/`security.md` dokumentieren Beispiele.
- **Konsequenzen:** Least-Privilege bis auf Subtree-Ebene mit minimalem Code (Wiederverwendung von `MatchSubject`), ohne neue Abhängigkeit. Kosten: Die Autorisierung ist nicht mehr rein in der Middleware — jede Datenroute braucht eine **explizite** Subjekt-Prüfung (Risiko des Vergessens → durch den zentralen Helfer + Tests je Route abgefangen). Bewusste Nicht-Ziele: keine **Deny**-Regeln (nur additive Grants), keine Wildcards in der Mitte (`/orders/*/items`), keine attributbasierte Policy, keine Tenancy-/Datenraum-Isolation über die Subtree-Sichtbarkeit hinaus (Subjects bleiben in **einer** DB; das ist **keine** Mandantentrennung — siehe `docs/threat-model.md`). Migration ist nahtlos: bestehende globale Scopes verhalten sich unverändert.
- **Bezug:** erweitert ADR-025 (Keys/Scopes), nutzt `MatchSubject`/Subject-Index (ADR-006/021/023). Implementierung als eigener, kleiner PR nach Freigabe dieses Designs.
