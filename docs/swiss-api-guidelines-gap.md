...
| Fehler-Body strukturiert, keine Stacktraces | MUST/SHOULD | `{"error": "..."}`, keine Stacktraces | ⚠️ kein `problem+json` | mittel, mechanisch |
| `application/problem+json` (RFC 7807) | SHOULD | nein | ⚠️ | mittel |
...
| `Cache-Control: no-store` als Default | MUST | keine Cache-Header | ❌ | **klein** |
...
## Billige, konfliktfreie Quick Wins (falls später gewünscht)

- Fehler als `application/problem+json` (RFC 7807): `{type,title,status,detail,invalid-params}`.
- OpenAPI-Meta ergänzen: `x-audience` (private/partner/public), `license`, `contact`.
- `Cache-Control: no-store` als Default-Header (Datenrouten).
